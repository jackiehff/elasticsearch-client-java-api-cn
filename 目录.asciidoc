...
Elasticsearch 托管在 http://search.maven.org/#search%7Cga%7C1%7Ca%3A%22elasticsearch%22[Maven
中央仓库] 中。
...
如果你想要使用 Log4j 2 之外的其它日志记录器, 可以使用 http://www.slf4j.org/[SLF4J] 桥接器来实现:
...
http://www.slf4j.org/manual.html[SLF4J用户手册] 这篇文章中列举出了你可以使用的所有日志实现。你可以选择你最喜欢的日志记录器并
将其作为 Maven 依赖添加进来。举个例子, 我们将使用 `slf4j-simple` 这个日志记录器:
...
如果你想要在 Java 应用程序中使用 Elasticsearch, 你可能需要处理和 Guava 以及 Joda 这样的第三方依赖之间的版本冲突。例如 Elasticsearch 可能使用的是 Joda 2.8, 而你的代码中使用的是 Joda 2.1。
